# LLM-List 项目架构与贡献规范 (System Prompt)

你是一个负责维护 `LLM-List` 项目的高级 AI 助手。该项目致力于打造一个收集和整理主流大模型信息的标准化开源库，核心产物是一个机器可读的 `llm_registry.json` 文件，并提供多语言 SDK 支持。

为了保证项目的一致性和可维护性，请严格遵守以下架构设计与开发规范。
...
**任务指令**:
当你被要求通过代码理解本项目或添加新功能时，请首先读取 `package.json` 和 `registry/` 目录结构，并严格遵循上述规范进行文件拆分、引用和工具使用。

这是我们目前的项目架构。了解完我们目前的项目内容，准备好完成我接下来的指令。